(function(){
  ...
  var firebaseConfig = {
    ...
    appId: "1:324916761555:web:9800e9d26685cbe672d526",
    measurementId: "G-69X37VGJPD"
  };

  var LS_COMMENTS = 'annot_comments_workbench_v1', LS_AUTHOR = 'annot_author';
  var useFB = false, store;
  var COL = 'comments_workbench';
  ...
  // an app modal/drawer = a large, visible, stacked (fixed/absolute) overlay that isn't our own UI
  // and is currently on-screen (so a slid-off / closed drawer is excluded)
  function isModalEl(el){
    if(!el || el.nodeType!==1) return false;
    if(el.classList && el.classList.contains('annot-ui')) return false;
    ...
    if(r.left > window.innerWidth-100 || r.right < 100 || r.top > window.innerHeight-60 || r.bottom < 60) return false;
    return true;
  }
  function findModal(el){
    while(el && el!==document.body){
      if(el.classList && el.classList.contains('annot-ui')) return null;
      if(isModalEl(el)) return el;
      el = el.parentElement;
    }
    return null;
  }
  ...
  function render(){
    ...
    shown.forEach(function(c, i){
      ...
      if(c.scope==='modal'){
        ...
        pin.style.position = 'absolute'; pin.style.left = c.x+'px'; pin.style.top = c.y+'px'; pin.style.transform = 'translate(-50%,-100%)';
        pin.innerHTML = badge;
  ...
  function buildToolbar(){
    ...
    toggleBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>';
  ...
  function init(){
    ...
    var s = document.createElement('style'); s.textContent = '.annot-pin{pointer-events:auto;}'+
      '@keyframes annotpop{from{opacity:0;transform:translateY(6px) scale(.98);}to{opacity:1;transform:none;}}'+
      '.annot-btn{height:32px;padding:0 13px;border:1px solid #d6dde6;background:#fff;color:#16233a;border-radius:8px;font:600 13px/1 Plus Jakarta Sans,system-ui,sans-serif;cursor:pointer;transition:background .12s,border-color .12s;}'+
      '.annot-btn:hover{background:#f6f8fa;border-color:#b9c3cf;}'+
      '.annot-btn.pri{background:#1668E3;border-color:#1668E3;color:#fff;}'+
      '.annot-btn.pri:hover{background:#1257c4;border-color:#1257c4;}'+
      '.annot-btn.danger{background:#fff;border-color:#f0c8c4;color:#b3271c;}'+
      '.annot-btn.danger:hover{background:#fdecea;border-color:#e0463b;}';
    document.head.appendChild(s);
    buildToolbar();

    document.addEventListener('click', function(e){
      if(!commentMode) return;
      if(e.target && e.target.nodeType===1 && e.target.closest('.annot-ui')) return;
      e.preventDefault(); e.stopPropagation();
      var modalEl = findModal(e.target && e.target.nodeType===1 ? e.target : null);
